...
│       │
│       ├── 📁 pronhub/                    # PRONHUB 平台模块
│       │   ├── 📄 __init__.py
│       │   └── 📄 pronhub.py              # ⭐ PRONHUB 视频抓取逻辑
│       │
│       └── 📁 javdb/                      # JAVDB 平台模块
│           ├── 📄 __init__.py
...
│  │              modules/ (功能模块)                          │   │
│  │  ┌────────────────┐ ┌──────────────┐ ┌──────────────┐  │   │
│  │  │  common/       │ │  pronhub/    │ │   javdb/     │  │   │
│  │  │ • common.py    │ │ • pronhub.py │ │ • javdb.py   │  │   │
│  │  │ • selenium_    │ └──────────────┘ └──────────────┘  │   │
│  │  │   helper.py    │                                     │   │
│  │  │ • error_       │                                     │   │
│  │  │   handler.py   │                                     │   │
...
└──────────┬───────────┘
           ...
           ▼                     ▼                     ▼
┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐
│ modules/common  │   │ modules/pronhub │   │ modules/javdb   │
│ • 文件扫描      │   │ • 网页抓取      │   │ • 网页抓取      │
│ • 缓存加载      │◄──┤ • 标题提取      │   │ • 标题提取      │
│ • 文件名清理    │   │ • 分页处理      │   │ • 分页处理      │
└────────┬────────┘   └────────┬────────┘   └────────┬────────┘
...
### 5. **modules/pronhub/pronhub.py** - PRONHUB 抓取
```python
主要功能：
├── fetch_with_requests_pronhub()     # 总入口（自动选择）
...
│   ├── 自动翻页
│   └── 错误处理
├── fetch_with_requests_only_pronhub() # Requests 抓取
│   ├── HTTP 请求
│   ├── BeautifulSoup 解析
│   ├── 提取标题
│   └── 自动翻页
├── clean_pronhub_title()             # 标题清理
├── match_pronhub_videos()            # 视频匹配
└── scan_pronhub_directory()          # 目录扫描
```
