...
class ModelManagerGUI:
    ...
    def init_model_tab(self):
        ...
        ttk.Button(action_frame, text="专属黑名单管理", command=self.open_model_blacklist_manager, width=20).pack(fill=tk.X, pady=5)
    ...
    def open_model_blacklist_manager(self):
        """打开当前模特的专属黑名单管理"""
        selected_items = self.model_tree.selection()
        if not selected_items:
            messagebox.showinfo("提示", "请选择要管理黑名单的模特")
            return
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title(f"{model_name} - 专属黑名单")
        ...
        info_label = ttk.Label(
            frame,
            text="在这里配置该模特需要跳过查重的URL。\n这些URL只对当前模特生效，查重时会自动忽略。",
            justify=tk.LEFT
        )
        info_label.pack(anchor=tk.W, pady=(0, 10))

        columns = ("url", "reason", "added_at")
        blacklist_tree = ttk.Treeview(frame, columns=columns, show="headings", height=10)
        blacklist_tree.heading("url", text="URL")
        blacklist_tree.heading("reason", text="原因")
        blacklist_tree.heading("added_at", text="添加时间")
        blacklist_tree.column("url", width=380)
        blacklist_tree.column("reason", width=180)
        blacklist_tree.column("added_at", width=120)

        scrollbar = ttk.Scrollbar(frame, orient=tk.VERTICAL, command=blacklist_tree.yview)
        blacklist_tree.configure(yscroll=scrollbar.set)
        blacklist_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        form_frame = ttk.LabelFrame(frame, text="新增规则", padding="10")
        form_frame.pack(fill=tk.X, pady=(12, 0))

        ttk.Label(form_frame, text="URL: ").grid(row=0, column=0, sticky=tk.W, pady=5)
        url_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=url_var, width=60).grid(row=0, column=1, sticky=tk.W, pady=5)

        ttk.Label(form_frame, text="原因: ").grid(row=1, column=0, sticky=tk.W, pady=5)
        reason_var = tk.StringVar()
        ttk.Entry(form_frame, textvariable=reason_var, width=60).grid(row=1, column=1, sticky=tk.W, pady=5)

        button_frame = ttk.Frame(form_frame)
        button_frame.grid(row=0, column=2, rowspan=2, padx=(10, 0))

        def refresh_list():
            for item_id in blacklist_tree.get_children():
                blacklist_tree.delete(item_id)
            try:
                from core.modules.common.model_database import ModelDatabase
                db = ModelDatabase('models.db')
                records = db.get_blacklist_records_by_model(model_name)
                for row in records:
                    blacklist_tree.insert("", tk.END, values=(row.get("url", ""), row.get("reason", ""), row.get("added_at", "")))
            except Exception as e:
                self.logger.warning(f"加载黑名单失败: {e}")
        ...
        def add_rule():
            ...
            try:
                from core.modules.common.model_database import ModelDatabase
                db = ModelDatabase('models.db')
                db.add_model_blacklist_url(model_name, new_url, reason)
                refresh_list()
                url_var.set("")
                reason_var.set("")
            except Exception as e:
        ...
        def remove_rule():
            ...
            if not selected:
                messagebox.showinfo("提示", "请选择要删除的URL规则")
                return
            item = blacklist_tree.item(selected[0], "values")
            url_to_remove = item[0]
            if not messagebox.askyesno("确认", "确定删除该URL规则吗？"):
                return
            try:
                from core.modules.common.model_database import ModelDatabase
                db = ModelDatabase('models.db')
                db.remove_model_blacklist_url(model_name, url_to_remove)
                refresh_list()
            except Exception as e:
                messagebox.showerror("错误", f"删除失败: {e}")

        ttk.Button(button_frame, text="添加", command=add_rule, width=10).pack(fill=tk.X, pady=2)
        ttk.Button(button_frame, text="删除选中", command=remove_rule, width=10).pack(fill=tk.X, pady=2)
        ttk.Button(button_frame, text="刷新", command=refresh_list, width=10).pack(fill=tk.X, pady=2)
